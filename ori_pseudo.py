...
from generalframeworks.networks.ddp_model import Model_ori_pseudo
...
def main(rank, config, args):
    ...
    model = Model_ori_pseudo(backbone, num_classes=config['Network']['num_class'], output_dim=256, config=config).cuda()
    ...
    criterion = {'sup_loss': ProbOhemCrossEntropy2d(ignore_label=-1, thresh=0.7, min_kept=50000 * config['Dataset']['batch_size']).cuda(),
                 ...
                 'unsup_loss': Attention_Threshold_Loss(strong_threshold=config['Loss']['un_threshold']).cuda(),
                 'contrast_loss': Contrast_Loss(strong_threshold=config['Loss']['strong_threshold'], 
...
def train(train_l_loader, train_u_loader, model, optimizer, criterion, epoch, scheduler, sche_d, config, args):
    ...
    for i, (train_l_image, train_l_label) in enumerate(train_l_loader):
        ...
        pred_l_large, pred_u_large, train_u_aug_label, train_u_aug_logits, rep_all, pred_all, pred_u_large_raw = model(train_l_image, train_u_image)
        ...
        with torch.no_grad():
            train_u_aug_mask = train_u_aug_logits.ge(config['Loss']['weak_threshold']).float()
        ...
        contrast_loss = criterion['contrast_loss'](rep_all, label_all, mask_all, prob_all, prototypes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--job_name', type=str, default='VOC_92_baseline')
