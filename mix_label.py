...
from generalframeworks.networks.ddp_model import Model_mix
...
from generalframeworks.utils import label_onehot, label_onehot_2
...
def main(rank, config, args):
    ...
    model = Model_mix(backbone, num_classes=config['Network']['num_class'], output_dim=256, config=config, temp=args.temp).cuda()
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        if epoch % 20 == 0 or epoch > total_epoch - 50:   # to save eval time, move it is ok
...
def train(train_l_loader, train_u_loader, model, optimizer, criterion, epoch, scheduler, sche_d, config, args):
    ...
    for i, (train_l_image, train_l_label) in enumerate(train_l_loader):
        ...
        pred_l_large, pred_u_large, train_u_aug_label, train_u_aug_logits_cls, train_u_aug_logits_rep, rep_all, pred_all = model(train_l_image, train_u_image, prototypes)

        if config['Dataset']['name'] == 'VOC':
            sup_loss = criterion['ce_loss'](pred_l_large, train_l_label)
        else:
            sup_loss = criterion['sup_loss'](pred_l_large, train_l_label)
        unsup_loss = criterion['unsup_loss'](pred_u_large, train_u_aug_label, train_u_aug_logits_cls)
        ...
        with torch.no_grad():
            ...
            label_u = F.interpolate(label_onehot_2(train_u_aug_label, num_class), size=pred_all.shape[2:], mode='nearest')
            label_u = label_u[:, 1:, :, :]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_iter', type=int, default=80000)
    parser.add_argument('--job_name', type=str, default='VOC_92_mix_label')
